...
import {  Button , Form , Upload ,Icon,  Breadcrumb , Modal , message ,Input , InputNumber , Layout ,Select ,DatePicker} from  'antd';
...
export default class Excel extends React.Component{
    ...
    // download = ()=>{
    //     axios.ajax({
    //         url: FaceUrl.bdApi+'/fileupload/exportExcel',
    //         method:FaceUrl.POST,
    //         baseApi:FaceUrl.bdApi
    //     }).then((res)=>{
    //         if(res.code == '1') {
    ...
    render(){
        ...
        const props = {
            name: 'file',
            action: FaceUrl.bdApi+'/fileupload/importExcel',
            ...
            beforeUpload(file){
                ...
                const flag = (filename.substring(filename.indexOf('.')) === '.xlsx')||(filename.substring(filename.indexOf('.')) === '.xls');
              if (!flag) {
               message.error('只能上传excel 文件!');
               return flag;
              }
              return flag;
            }
            ...
            <div>
                ...
                <Content className="content-wrap">
                    ...
                    <span className="table_button ht" >
                        ...
                        <Upload {...props} onChange={this.handleOnChange}>
                         <Button ghost type="primary">
                             <Icon type="upload" /> Excel导入
                         </Button>
                       </Upload>
                       
                       <Button icon="download" ghost type="primary"><a style={{'text-decoration':'none'}} href={FaceUrl.bdApi+'/fileupload/exportExcel'}>Excel导出</a></Button> 
                    </span>   
                    </div>        
                    <ETable
                        columns={columns}
